...
public class Ex05For {
    ...
    public static void main(String[] args) {
        ...
        // 2. 제어 변수(i)는 정말로 변수이다. 다만 유효 범위는 해당 for문 안이 된다.
        for ( int i = 1; i <= 3; i ++) {
            System.out.println(i);
        }
        // 3. 만약 처음부터 반복 조건식이 false가 나오면 while문과 마찬가지로 코드는 실행되지않는다.
        for ( int i = 1; i<0 ; i ++) {
            System.out.println("실행될까?");
        }
        // 4. 제어변수의 변화식은 ++, -- 뿐만이 아니라 다른 할당 연산자를 사용할 수도 있다.
        for( int i = 1; i < 5000 ; i *= 100) {
            System.out.println(i); // *= 100 >> 1, 100 , 10000
        }
        
        // 5. 제어변수의 선언과 초기화, 조건식, 변화식을 모두 for문의 ()에서
        // 분리시킬수도 있지만 이 방법은 최대한 사용을 자제하는것이 좋다.
        // 단, 분리되더라도, ; 의갯수는 반드시 ()안에 2개가 있어야한다.
        int i = 11;
        for (; ; ){
            if ( i > 13) {
                break;
