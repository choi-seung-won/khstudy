...
// 내가 어제 구현한 기능을 오늘 다시 쓸려고 보니
// 처음부터 다시 구현했어야 했고, 그것이 "귀찮아서"
// 기능을 재사용하는 2세대 절차지형적 언어가 나왔던 것이다.
// 하지만 처음과는 달리 단순히 기능만 재사용하는것보다
// 좀 더 복합적인 개념을 재사용하게 사람들이 생각하게 됬고
// 그렇게 3세대 언어 "객체 지향적 언어(Object Oriented Programming)"이 탄생하게 된 것이다.

// 객체란 무엇인가?

// 객체는 클래스의 변수를 뜻한다.
// 즉 객체 지향 프로그래밍 에서는 우리가 단순히 데이터를 저장할 구조체 
// 혹은 기능을 단독적으로 재사용하는것이 아니라
// 그 모든것이 합쳐져 있는 클래스 자체를 통째로 재사용하는것을 지향한다.
...
// 자바의 모든 클래스는 java.lang.Object를 직 간접적으로 상속을 받게 된다
// 직접적으로 상속을 받는 경우는 바로 윗 부모가 java.lang.Object인 경우이고
// 간접적으로 상속을 받는 경우는 윗부모가 다른 클래스이지만 최종적으로 가장 높은부모가 java.lang.Object인
// 경우이다.


// 단 , 부모 클래스가 물려준 메소드가 반드시 자식 클래스가 쓰기 편하다는 보장은 없다.
// 따라서 필요에 따라서는 부모 클래스가 물려준 메소드 대신 자식 클래스가 똑같은 이름의 메소드를
// "재정의" 해주어야 하는데 이러한 메소드의 재정의를 우리가 "오버라이드(Override)"라고 부른다.
...
// 이름을 통일 시킴으로써, 우리가 똑같은 목표를 가진 메소드를 다양한 파라미터에 대비해줄 수 있게 된다.
...
// 이 기본 생성자의 경우, 해당 클래스변수의 필드를 초기화만 하는 역할을 한다.
// 초기화시에, 만약 해당 필드가 기본형 데이터타입이면 0으로 초기화를 하고,
// 참조형 데이터타입의 필드일 경우에는 null로 초기화한다.

// 필요에 따라서는 우리가 직접 생성자를 만들어 줄 수도 있는데
// 이 때 주의할점은 만약 파라미터가 있는 생성자만 만들어주면
...
public class Ex01Object {
    ...
    public static void main(String[] args) {
        ...
        // Student 객체 선언 및 초기화
        Student s = new Student(30, " aaa " , 20 , 20 , 21);//public Student로 따로 생성자를 만들었기에 자동으로 할당되지않음 따라서 생성자에 맞는 파라미터를 지정해줘야함.
        // 초기화 직후 저장된 값 출력
        s.printInfo();
        // 필드에 값 저장
        s.id = 1;
        ...
        // Student 객체 s3 생성 후 파라미터가 있는 생성자를 통해 똑같은 값 입력
