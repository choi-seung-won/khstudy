...
//코드로써 주로 조건문 혹은 반복문에서 코드의 실행여부 또는 반복여부를 결정하는데
//사용된다.
//단순히 비교연산자 혹은 논리연산자만 사용하는것이 아니라
//equals()와 같이 결과값이 boolean이면 뭐든 사용할 수 있다.

//코드 블락
//코드 블락이란 한개의 중괄호 안에 있는 모든 코드들을
//코드 블락이라고 한다.
//코드 블락 안에는 다른 여러개의 코드 블락이 들어올 수도 있으며
//해당 여러개의 코드 블락을 우리는 "자식" 코드 블락이라고 표현한다
//또한 해당 자식코드블락들은 부모 코드블락의 코드로 취급이 된다.

//변수의 유효범위(scope)
//변수의경우, 유효범위라는 것이 설정되어있으며
//해당 유효범위를 벗어나면, 더이상 그 변수는 존재하지 않는 것으로 취급이 된다.
//또한, 해당 변수가 유효한 범위 안에서는 똑같은 이름의 변수는 선언할 수 없다.
//변수가 유효한 범위는 해당 변수를 선언한 이후부터
//그변수가 속해있는 코드 블락이 끝나기 전까지이다.

//조건문 01
//if 조건문은 특정 조건식이
//true가 나오면 연결되어 있는 코드 블락을 실행시키고,
//만약 false가 나오면 해당 코드블락을 무시하는 제어문이다.
...
public class Ex03If01 {
    public static void main(String[] args) {
        ...
        if (number >= 0) {
            ...
            // int number =7;Duplicate local variable number
            int number2 = 7;
